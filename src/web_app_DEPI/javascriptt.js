document.getElementById('predictButton').addEventListener('click', function() {
    ...
    let yearsInCurrentRole = parseFloat(data.yearsInCurrentRole);
    let yearsSinceLastPromotion = parseFloat(data.yearsSinceLastPromotion);
    let yearsWithCurrManager = parseFloat(data.yearsWithCurrManager);
    ...
    if (isNaN(totalWorkingYears) || totalWorkingYears < 0 || totalWorkingYears > age - 14 || !Number.isInteger(totalWorkingYears)) {
        errors.push('Total Working Years must be an integer between 0 and ' + (age - 14));
    }
    ...
    if (isNaN(yearsInCurrentRole) || yearsInCurrentRole < 0 || yearsInCurrentRole > yearsAtCompany || !Number.isInteger(yearsInCurrentRole)) {
        errors.push('Years in Current Role must be an integer between 0 and Years at Company');
    }
    ...
    if (isNaN(yearsWithCurrManager) || yearsWithCurrManager < 0 || yearsWithCurrManager > yearsAtCompany || !Number.isInteger(yearsWithCurrManager)) {
        errors.push('Years with Current Manager must be an integer between 0 and Years at Company');
    }
    ...
    if (isNaN(percentSalaryHike) || percentSalaryHike < 0 || percentSalaryHike > 100) {
